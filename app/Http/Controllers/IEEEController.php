...
class IEEEController extends Controller {    
    ...
    public function import_json() {
        ...
        $query = null;
        $path_file = storage_path() . "/data_files/ieee/json/";
        ...
        foreach($files as $file) 
        {
            ...
            foreach($articles as $key => $article) {  
                ...
                $article["pdf_link"]        = Config::get('constants.pach_ieee') . "xpl/abstractSimilar.jsp?arnumber=" . $article["source_id"];
                $article["document_url"]    = Config::get('constants.pach_ieee') . "document/" . $article["source_id"];
                $article["source"]          = Config::get('constants.source_ieee');
                $article["source_id"]       = $article["source_id"];
                $article["citation-key"]       = $article["source_id"];
                $article["bibtex"]       = null;
    ...
    public function import_bibtex() {
        ...
        $query = '(("AAL" OR "Ambient Assisted Living" OR "ambient assisted" OR "assisted living" OR "ambient assistance" OR "assisted environment" OR "assistive environment" OR "AAL environment" OR "independent living" OR "assisted life" OR "intelligent living" OR "pervasive living" OR "assistive environments" OR "AAL environments" OR "assisted environments") AND ("quality model" OR "quality attribute" OR "non-functional property" OR "Non-Functional Requirements" OR "NFR" OR "quality requirement" OR "quality models" OR "quality attributes" OR "non-functional properties" OR "non-functional requirements" OR "quality requirements" OR "NFR") AND (specification OR model))';
